...
describe GameCommands do
  ...
  context 'correct_initial_command_check' do
    it 'should return true if the user response includes correctly formatted PLACE X,Y,F command' do
      expect(game_commands.correct_initial_command_check('PLACE 0,0,WEST')).to be true
    end

    it 'should return false if the user response includes any string other than the PLACE command' do
      expect(game_commands.correct_initial_command_check('MOVE')).to be false
      expect(game_commands.correct_initial_command_check('RIGHT')).to be false
      expect(game_commands.correct_initial_command_check('LEFT')).to be false
      expect(game_commands.correct_initial_command_check('REPORT')).to be false
    end
  end
